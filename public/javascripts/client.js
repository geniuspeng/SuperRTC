/**
 * Created by baiyp on 2015/12/16.
 */
var SuperRTC = function() {
    ...
    superrtc.prototype = new EventEmitter();
    ...
    /*************************服务器连接部分***************************/
        ...
        //本地连接信道，信道为websocket
    superrtc.prototype.connect = function(server, room) {
        var socket,
            that = this;
        room = room || "";
    ...
    /*************************流处理部分*******************************/
        ...
        //创建本地流
    superrtc.prototype.createStream = function(options) {
        var that = this;
    ...
    superrtc.prototype.addStreams = function() {
        var i, m,
            stream,
            connection;
    ...
    //将流绑定到video标签上用于输出
    superrtc.prototype.attachStream = function(stream, domId) {
    ...
    superrtc.prototype.sendOffers = function() {
        var i, m,
            pc,
            that = this,
    ...
    superrtc.prototype.receiveOffer = function(socketId, sdp) {
        var pc = this.peerConnections[socketId];
        this.sendAnswer(socketId, sdp);
    };

    //发送answer类型信令
    superrtc.prototype.sendAnswer = function(socketId, sdp) {
        var pc = this.peerConnections[socketId];
        var that = this;
    ...
    superrtc.prototype.receiveAnswer = function(socketId, sdp) {
        var pc = this.peerConnections[socketId];
        pc.setRemoteDescription(new nativeRTCSessionDescription(sdp));
    };
    ...
    superrtc.prototype.createPeerConnections = function() {
        var i, m;
        for (i = 0, m = this.connections.length; i < m; i++) {
    ...
    //创建单个PeerConnection
    superrtc.prototype.createPeerConnection = function(socketId) {
        var that = this;
    ...
    //关闭PeerConnection连接
    superrtc.prototype.closePeerConnection = function(pc) {
        if (!pc) return;
        pc.close();
    };
    ...
    /***********************数据通道连接部分*****************************/
        ...
        //消息广播
    superrtc.prototype.broadcast = function(message) {
        var socketId;
    ...
    //发送消息方法
    superrtc.prototype.sendMessage = function(message, socketId) {
    ...
    //共享板信息广播
    superrtc.prototype.broadcastBoardInfo = function (message) {
        var socketId;
        for (socketId in this.dataChannels) {
            this.sendBoardMessage(message, socketId);
        }
    }
    //发送共享版消息方法
    superrtc.prototype.sendBoardMessage = function(message, socketId) {
        if (this.dataChannels[socketId].readyState.toLowerCase() === 'open') {
            this.dataChannels[socketId].send(JSON.stringify({
                type: "__draw",
                data: message
    ...
    superrtc.prototype.addDataChannels = function() {
        var connection;
        for (connection in this.peerConnections) {
    ...
    superrtc.prototype.createDataChannel = function(socketId, label) {
        var pc, key, channel;
    ...
    superrtc.prototype.addDataChannel = function(socketId, channel) {
        var that = this;
        ...
        channel.onmessage = function(message) {
            ...
            }else if(json.type === '__draw'){
                that.emit('draw', json.data);
            } else {
    ...
    superrtc.prototype.parseFilePacket = function(json, socketId) {
    ...
    superrtc.prototype.shareFile = function(dom) {
        var socketId,
            that = this;
    ...
    //向某一单个用户发送文件
    superrtc.prototype.sendFile = function(dom, socketId) {
        var that = this,
            file,
            reader,
            fileToSend,
    ...
    //发送多个文件的碎片
    superrtc.prototype.sendFileChunks = function() {
        var socketId,
            sendId,
            that = this,
            nextTick = false;
    ...
    //发送某个文件的碎片
    superrtc.prototype.sendFileChunk = function(socketId, sendId) {
        var that = this,
    ...
    //发送文件请求后若对方同意接受,开始传输
    superrtc.prototype.receiveFileAccept = function(sendId, socketId) {
        var that = this,
            fileToSend,
            reader,
    ...
    superrtc.prototype.receiveFileRefuse = function(sendId, socketId) {
        var that = this;
    ...
    //清除发送文件缓存
    superrtc.prototype.cleanSendFile = function(sendId, socketId) {
        var that = this;
        delete that.fileChannels[socketId][sendId];
    };

    //发送文件请求
    superrtc.prototype.sendAsk = function(socketId, sendId, fileToSend) {
        var that = this,
            channel = that.dataChannels[socketId],
            packet;
        if (!channel) {
    ...
    //获得随机字符串来生成文件发送ID
    superrtc.prototype.getRandomString = function() {
    ...
    /***********************接收者部分***********************/
        ...
        //接收到文件碎片
    superrtc.prototype.receiveFileChunk = function(data, sendId, socketId, last, percent) {
        var that = this,
    ...
    superrtc.prototype.getTransferedFile = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
    ...
    //接收到发送文件请求后记录文件信息
    superrtc.prototype.receiveFileAsk = function(sendId, fileName, fileSize, socketId) {
        var that = this;
    ...
    //发送同意接收文件信令
    superrtc.prototype.sendFileAccept = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
    ...
    //发送拒绝接受文件信令
    superrtc.prototype.sendFileRefuse = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
            channel = that.dataChannels[fileInfo.socketId],
            packet;
    ...
    //清除接受文件缓存
    superrtc.prototype.cleanReceiveFile = function(sendId) {
        var that = this;
